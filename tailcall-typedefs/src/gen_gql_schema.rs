...
use tailcall::core::config::{
    AddField, Alias, Cache, Call, Expr, GraphQL, Grpc, Http, Link, Modify, Omit,  Protected,  Server,  Tag, Telemetry, Upstream, JS
};
...
use tailcall_typedefs_common::DirectiveDefinition;
use tailcall_typedefs_common::InputDefinition;
use tailcall_typedefs_common::ScalarDefinition;
...
pub async fn update_gql(file_io: Arc<dyn FileIO>) -> Result<()> {
    ...
    let doc = builder
        ...
        .add_input(Upstream::input_definition())
