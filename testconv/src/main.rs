use std::{collections::HashSet, fs::{self, canonicalize, read_dir, File}, io::Write, path::PathBuf};
...
fn main() {
    let http_dir = canonicalize(PathBuf::from("tests/http"))
        .expect("Could not find http directory");

    let execution_dir = canonicalize(PathBuf::from("tests/execution"))
        .expect("Could not find execution directory");

    let snapshots_dir = canonicalize(PathBuf::from("tests/snapshots"))
        .expect("Could not find snapshots directory");
    ...
    for x in read_dir(http_dir).expect("Could not read http directory") {
        ...
        let path = x.path();
        let file_stem = path.file_stem().unwrap().to_string_lossy().to_owned().to_string();

        if files_already_processed.contains(&file_stem) {
            panic!("File name collision: {}", file_stem);
        }

        if path.is_file() && path.extension().unwrap().to_str().unwrap().to_string().as_str() == "yml" {
            ...
            spec += &format!(
                ...
                serde_yaml::to_string(
                    &execution::AssertSpec::from(old.clone())
            ...
            let mut f = File::options()
                ...
                .truncate(true)
                .open(
                    execution_dir.join(PathBuf::from(format!("{}.md", file_stem)))
                )
                .expect("Failed to open execution spec");

            f.write_all(spec.as_bytes()).expect("Failed to open execution spec");
            ...
            for (i, assert) in old.assert.into_iter().enumerate() {
                let mut f = File::options()
                    ...
                    .truncate(true)
                    .open(
                        snapshots_dir.join(PathBuf::from(format!(
                ...
                if !res.0.headers.contains_key("content-type") {
                    res.0.headers.insert("content-type".to_string(), "application/json".to_string());
                }
