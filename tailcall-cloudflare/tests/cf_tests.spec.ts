...
describe("fetch", () => {
  test("loadfiles", async () => {
    ...
    let news_proto = (await readFile("../tailcall-fixtures/grpc/proto/news.proto")).toString()

    let bucket = await mf.getR2Bucket("MY_R2")
    await bucket.put("examples/grpc.graphql", grpc)
    await bucket.put("examples/../tailcall-fixtures/grpc/proto/news.proto", news_proto)
    await bucket.put("tailcall-fixtures/grpc/proto/news.proto", grpc)
