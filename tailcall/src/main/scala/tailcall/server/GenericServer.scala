...
object GenericServer {
  def graphQL(
    ...
    Http.collectZIO[Request] {
      case req @ method -> !! / "graphql"       => executeGraphQL(timeout, req, method, None)
      case req @ method -> !! / "graphql" / hex => executeGraphQL(timeout, req, method, Some(hex))

    }

  private def executeGraphQL(timeout: zio.Duration, req: Request, method: Method, hex: Option[String]) = {
    for {
      option      <- hex.fold(InterpreterRegistry.getDefault)(InterpreterRegistry.get(_))
