...
describe("fetch", () => {
  test("loadfiles", async () => {
    ...
    let news_proto = (await readFile("../src/grpc/tests/proto/news.proto")).toString()

    let bucket = await mf.getR2Bucket("MY_R2")
    await bucket.put("examples/grpc.graphql", grpc)
    await bucket.put("examples/../src/grpc/tests/proto/news.proto", news_proto)
    await bucket.put("src/grpc/tests/proto/news.proto", grpc)
