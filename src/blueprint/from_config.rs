...
impl<'a> MustachePartsValidator<'a> {
    ...
    fn get_nested_type(&self, tail: &[String], type_of: &config::Type) -> Result<Type, String> {
        ...
        let field = type_of.fields.get(head).ok_or(format!("no value '{}' found",tail[0].as_str()))?;
        let type_of = self.config.find_type(&field.type_of).ok_or(format!("no value '{}' found",tail[0].as_str()))?;
        ...
        match head {
            "value" => {
                ...
                let val_type = if tail_is_long {
                    ...
                    get_value_type(type_of, tail).ok_or(format!("no value '{tail}' found",))
                };
