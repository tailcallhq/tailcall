...
fn compile(ctx: &CompilationContext, expr: ExprBody) -> Valid<Expression, String> {
    ...
    match expr {
        ...
        ExprBody::Mean(ref list) => compile_list(ctx, list.clone())
            .map(|a| Expression::Math(Math::Mean(a)).parallel_when(expr.has_io())),
        ExprBody::Median(ref list) => compile_list(ctx, list.clone())
            .map(|a| Expression::Math(Math::Median(a)).parallel_when(expr.has_io())),
    }
}
...
mod tests {
    ...
    async fn test_math_mean() {
        ...
        let actual = Expr::eval(
            json!({"body": {"mean": [{"const": 1}, {"const": 2}, {"const": 3}, {"const": 4}]}}),
        )
        .await
        .unwrap();
