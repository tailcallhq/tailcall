...
impl CorsParams {
    ...
    pub fn allow_private_network_to_header(&self, parts: &Parts) -> Option<(HeaderName, HeaderValue)> {
        ...
        self.allow_private_network.then(|| (ALLOW_PRIVATE_NETWORK, TRUE))
    }
    ...
    pub fn const_or_mirror_to_header(&self, const_or_mirror: &ConstOrMirror, parts: &Parts) -> Option<(HeaderName, HeaderValue)> {
    ...
    pub fn max_age_to_header(&self) -> Option<(HeaderName, HeaderValue)> {
        Some((header::ACCESS_CONTROL_MAX_AGE, self.max_age.as_ref()?.clone()))
    }

    pub fn expose_headers_to_header(&self) -> Option<(HeaderName, HeaderValue)> {
        Some((header::ACCESS_CONTROL_EXPOSE_HEADERS, self.expose_headers.as_ref()?.clone()))
    }
}
...
impl TryFrom<config::cors_params::CorsParams> for CorsParams {
    ...
    fn try_from(value: config::cors_params::CorsParams) -> anyhow::Result<Self> {
        Ok(CorsParams {
            allow_credentials: value.allow_credentials.map(|val| val.into()),
            allow_headers: value.allow_headers.try_into()?,
            allow_methods: value.allow_methods.try_into()?,
            allow_origin: value.allow_origin.iter().map(|val| Ok(val.parse()?)).collect::<anyhow::Result<_>>()?,
            allow_private_network: false,
            expose_headers: value.expose_headers.map(|val| val.parse()).transpose()?,
            max_age: value.max_age.map(|val| val.into()),
            vary: value.vary.iter().map(|val| Ok(val.parse()?)).collect::<anyhow::Result<_>>()?,
        })
    }
}
