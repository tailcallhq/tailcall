...
fn compile_if(context: &CompilationContext, condition: Box<ExprBody>, then: Box<ExprBody>, els: Box<ExprBody>) -> Valid<Expression, String> {
    compile(context, *condition).map(Box::new)
...
pub fn update_expr(
  ...
  TryFold::<(&Config, &Field, &config::Type, &str), FieldDefinition, String>::new(
    |(config, field, ty, name), b_field| {
      ...
      compile(&context, expr.body.clone())
          .map(|compiled| b_field.resolver(Some(compiled)))
    }
  )
}
