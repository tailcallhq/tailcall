    ...
    pub fn new(loader: T) -> Self {
        Self {
            inner: Arc::new(DataLoaderInner {
                requests: Mutex::new(Requests::new(&NoCache)),
                loader,
            }),
...
impl<K: Send + Sync + Hash + Eq + Clone + 'static, T: Loader<K>, C: CacheFactory<K, T::Value>>
    Requests<K, T, C>
{
...
mod tests {
    ...
    use super::*;
    use crate::data_loader::cache::HashMapCache;
    ...
    async fn test_dataloader_dead_lock() {
        ...
        let loader =
            Arc::new(DataLoader::with_cache(MyDelayLoader, NoCache).delay(Duration::from_secs(1)));
