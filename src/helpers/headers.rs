...
pub fn to_header_vec(headers: &KeyValues) -> Valid<MustacheHeaders, String> {
...
mod tests {
  ...
  fn valid_headers() -> Result<()> {
    ...
    let headers = to_header_vec(&input).to_result()?;
  ...
  fn not_valid_due_to_utf8() {
    ...
    let error = to_header_vec(&input).to_result().unwrap_err();
