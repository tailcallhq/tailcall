  ...
  def spec =
    suite("RemoteSchemaInfer")(
      ...
      suite("string")(test("concat") {
        val program =
          SchemaInferer.inferSchema(Lambda("hello") ++ Lambda("world"))
        assertTrue(program == Schema[String])
      }),
      suite("tuple")(
        test("get Index") {
      ...
      suite("sequence")(
        test("fromSeq") {
          val program =
            SchemaInferer.inferSchema(Lambda.fromSeq(Seq(Lambda(1), Lambda(2))))
        ...
        test("map") {
          val program =
            SchemaInferer.inferSchema(Lambda(Seq(1, 2, 3)).map(_ + Lambda(1)))
      ...
      suite("either")(
        test("right") {
          val program =
            SchemaInferer.inferSchema(Lambda.fromEither(Right(Lambda(1))))
          assertTrue(program.ast == Schema[Either[Unit, Int]].ast)
        },
        test("left") {
          val program =
            SchemaInferer.inferSchema(Lambda.fromEither(Left(Lambda(1))))
