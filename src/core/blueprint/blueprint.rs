...
mod tests {
    ...
    fn assert_default_value<T: std::fmt::Debug + Serialize>(field_type: &str, default_value: T) {
        let sdl = format!(
            r#"
        ...
        let config_module = ConfigModule::from(Config::from_sdl(&sdl).to_result().unwrap());
        let blueprint = Blueprint::try_from(&config_module).unwrap();
        assert!(blueprint.definitions.iter().any(|x| match x {
            Definition::InputObject(obj) => {
                obj.name == "InputPost"
                    && obj.fields.iter().any(|fld| {
                        fld.name == "id"
                            && fld.default_value == Some(serde_json::json!(default_value))
                    })
            }
            _ => false,
        }))
    ...
    #[test]
    fn test_input_field_default_value_int() {
        assert_default_value("Int", 1001);
    }

    #[test]
    fn test_input_field_default_value_string() {
        assert_default_value("String", "1001");
    }

    #[test]
    fn test_input_field_default_value_float() {
        assert_default_value("Float", 1001.0);
    }
}
