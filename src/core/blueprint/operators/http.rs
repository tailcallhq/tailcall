...
pub fn compile_http(
    ...
    let mustache_headers = match helpers::headers::to_mustache_headers(&http.headers).to_result() {
        ...
        Err(e) => Valid::fail(BlueprintError::Mustache(e)),
    };

    Valid::<(), BlueprintError>::fail(BlueprintError::Validation(
        "GroupBy is only supported for GET requests".to_owned(),
    ))
    .when(|| !http.batch_key.is_empty() && http.method != Method::GET)
    .and(
        Valid::<(), BlueprintError>::fail(BlueprintError::Validation(
            "Batching capability was used without enabling it in upstream".to_owned(),
        ))
        .when(|| {
