...
pub fn compile_http(config_module: &config::ConfigModule, http: &config::Http) -> Valid<IR, String> {
  Valid::<(), String>
    ::fail("GroupBy is only supported for GET requests".to_string())
    .when(|| !http.batch_key.is_empty() && http.method != Method::GET)
    .and(
      Valid::<(), String>
        ::fail("Batching capability was used without enabling it in upstream".to_string())
        .when(|| {
    ...
    .and(
      Valid::from_option(
        http.base_url.as_ref().or(config_module.upstream.base_url.as_ref()),
    ...
    .map(|req_template| {
      ...
      let http_filter = http.on_request
        .clone()
  ...
  TryFold::<(&ConfigModule, &Field, &config::Type, &'a str), FieldDefinition, String>::new(
    |(config_module, field, type_of, _), b_field| {
      ...
      compile_http(config_module, http)
        ...
        .and_then(|b_field| { b_field.validate_field(type_of, config_module).map_to(b_field) })
    }
  )
}
