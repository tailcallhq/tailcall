...
impl BlueprintMetadata {
    ...
    pub fn schema_definition(&self) -> Valid<SchemaDefinition, super::Error> {
...
// on each step
    pub fn to_bp_schema_def(&self, schema_definition: &SchemaDefinition) -> Valid<blueprint::SchemaDefinition, super::Error> {
        helpers::extract_directives(schema_definition.directives.iter())
            .fuse(self.validate_query(schema_definition.query.as_ref().map(pos_name_to_string)))
    ...
    fn validate_query(&self, qry: Option<String>) -> Valid<String, super::Error> {
        Valid::from_option(qry, "Query root is missing".to_owned())
            .and_then(|query_type_name| {
                // println!("{:#?}", schema_definition);
                let find = self.doc.definitions.iter().find_map(|directive| {
