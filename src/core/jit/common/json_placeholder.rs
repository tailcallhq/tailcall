...
use crate::core::json::{JsonLike, JsonT, JsonObjectLike};
...
impl JsonPlaceholder {
    ...
    fn value<Value: JsonT + Clone + serde::Deserialize<'static>>() -> TestData<Value> {
        ...
        let user_map = users.iter().fold(HashMap::new(), |mut map, user| {
            let id = Value::object_ok(user).and_then(|v| v.get("id").and_then(|u| u.as_u64_ok().ok()));
    ...
    fn data<
        Value: JsonT + Clone + serde::Deserialize<'static>
    >(
        plan: &ExecutionPlan,
        ...
        let store = [
            (
                posts_id,
                Data::Single(<Value as JsonT>::new_array(posts)),
            ),
    ...
    pub fn init<
        Value: JsonT + Deserialize<'static> + Clone,
        T: SynthExt<Value>
    >(query: &str) -> Box<T> {
...
mod tests {
    ...
    #[test]
    fn test_user() {
        let synth: Box<SynthBorrow> = JsonPlaceholder::init("{ posts { id title userId user { id name } } }");
