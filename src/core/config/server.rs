...
pub struct Server {
    ...
    /// - `Some(true)`: Enables JIT compilation, allowing dynamic code optimization.
    /// - `Some(false)` or `None`: Disables JIT compilation, relying on default execution mechanisms.
    /// 
