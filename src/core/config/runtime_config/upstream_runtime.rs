...
impl Default for UpstreamRuntime {
    fn default() -> Self {
        // NOTE: Using unwrap because try_from default will never fail
        UpstreamRuntime::try_from(&ConfigModule::default()).unwrap()
    }
}

impl TryFrom<&ConfigModule> for UpstreamRuntime {
    ...
    fn try_from(config_module: &ConfigModule) -> Result<Self, Self::Error> {
        ...
        let upstream_static = UpstreamStatic::from(config_upstream.clone());
        get_batch(&upstream_static)
            .fuse(get_proxy(&upstream_static))
            .map(|(batch, proxy)| UpstreamRuntime {
...
fn get_batch(upstream: &UpstreamStatic) -> Valid<Option<Batch>, String> {
...
fn get_proxy(upstream: &UpstreamStatic) -> Valid<Option<Proxy>, String> {
