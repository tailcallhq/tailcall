...
impl Transform for RenameTypes {
    ...
    fn transform(&self, config: Self::Value) -> Valid<Self::Value, Self::Error> {
        ...
        Valid::from_iter(self.0.iter(), |(existing_name, suggested_name)| {
            if config.blueprint_builder.types.contains_key(existing_name)
                || config.blueprint_builder.enums.contains_key(existing_name)
                || config.blueprint_builder.unions.contains_key(existing_name)
            {
                // handle for the types.
                if let Some(type_info) = config.blueprint_builder.types.remove(existing_name) {
                    config
                        .blueprint_builder
                        .types
                        .insert(suggested_name.to_string(), type_info);
                    lookup.insert(existing_name.clone(), suggested_name.clone());

                    // edge case where type is of operation type.
                    if config.blueprint_builder.schema.query == Some(existing_name.clone()) {
                        config.blueprint_builder.schema.query = Some(suggested_name.clone());
                    } else if config.blueprint_builder.schema.mutation
                        == Some(existing_name.clone())
                    {
                        config.blueprint_builder.schema.mutation = Some(suggested_name.clone());
                    }
                }

                // handle for the enums.
                if let Some(type_info) = config.blueprint_builder.enums.remove(existing_name) {
                    config
                        .blueprint_builder
                        .enums
                        .insert(suggested_name.to_string(), type_info);
                ...
                if let Some(type_info) = config.blueprint_builder.unions.remove(existing_name) {
                    config
                        .blueprint_builder
                        .unions
                        .insert(suggested_name.to_string(), type_info);
        ...
        .map(|_| {
            for type_ in config.blueprint_builder.types.values_mut() {
