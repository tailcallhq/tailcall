    ...
    fn transform(&self, mut config: Self::Value) -> Valid<Self::Value, Self::Error> {
        ...
        Valid::from_iter(input_types.intersection(&output_types), |current_name| {
            ...
            if !resolution.is_unique() {
                ...
                if let Some(ty) = config.blueprint_builder.types.get(current_name) {
        ...
        .map(|resolution_map| {
            ...
            for (current_name, resolution) in &resolution_map {
                ...
                let og_ty = config.blueprint_builder.types.get(current_name).cloned();

                // remove old types
                config.blueprint_builder.types.remove(current_name);
                ...
                if let Some(og_ty) = og_ty {
                    config
                        .blueprint_builder
                        .types
                        .insert(input_name.clone(), og_ty.clone());
                    input_types.insert(input_name.clone());

                    config
                        .blueprint_builder
                        .types
                        .insert(output_name.clone(), og_ty);
            ...
            for k in keys {
                if let Some(ty) = config.blueprint_builder.types.get_mut(&k) {
...
mod tests {
    ...
    fn test_resolve_ambiguous_types() {
        ...
        config
            .blueprint_builder
            .types
            .insert("Type1".to_string(), type1);
        config
            .blueprint_builder
            .types
            .insert("Type2".to_string(), type2);
        config
            .blueprint_builder
            .types
            .insert("Type3".to_string(), type3);
