...
pub struct Upstream {
    ...
    #[serde(default, skip_serializing_if = "is_default")]
    /// The type of cache to be used. The two options are `ByteBased` and `NumberOfEntryBased`. 
    /// If not specified, the default is `NumberOfEntryBased`.
