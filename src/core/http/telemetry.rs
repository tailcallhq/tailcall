...
static HTTP_SERVER_REQUEST_COUNT: Lazy<Counter<u64>> = Lazy::new(|| {
    ...
    meter
        ...
        .with_description("Number of Bytes request handled")
        .init()
});
...
impl RequestCounter {
    pub fn new(telemetry: &Telemetry, req: &Request<Bytes>) -> Self {
    ...
    pub fn update(self, response: &Result<Response<Bytes>>) {
...
pub fn get_response_status_code(response: &Response<Bytes>) -> KeyValue {
    KeyValue::new(HTTP_RESPONSE_STATUS_CODE, response.status().as_u16() as i64)
}

pub fn propagate_context(req: &Request<Bytes>) {
