...
pub struct RequestTemplate {
    ...
    pub body_path: Option<DynamicValue<async_graphql_value::ConstValue>>,
...
impl RequestTemplate {
    ...
    fn create_url<C: PathString + PathValue>(&self, ctx: &C) -> anyhow::Result<Url> {
        ...
        let extra_qp = self.query.iter().filter_map(|query| {
            ...
            let parsed_value = if skip {
                parsed_value.and_then(|v| v.skip_null())
            } else {
                parsed_value
            };
    ...
    pub fn to_request<C: PathString + HasHeaders + PathValue>(
        ...
        req = self.set_body(req, ctx, value_length)?;

        Ok(req)
    }

    /// Sets the body for the request
    fn set_body<C: PathString + PathValue + HasHeaders>(
        &self,
        ...
        if let Some(body_path) = &self.body_path {
            let rendered_body = if batch_size > 0 {
                Expander::<DynamicValue<async_graphql_value::ConstValue>>::expand(
                    body_path.clone(),
                    batch_size,
                )?
                .render_value(ctx)
            } else {
                body_path.render_value(ctx)
            };
            let rendered_body = serde_json::to_string(&rendered_body)?;
            match &self.encoding {
...
impl<Ctx: PathString + HasHeaders + PathValue> CacheKey<Ctx> for RequestTemplate {
    fn cache_key(&self, ctx: &Ctx) -> Option<IoId> {
        ...
        if let Some(body) = self.body_path.as_ref() {
            body.render_value(ctx).to_string().hash(state);
        }
...
struct TestEval<A>(std::marker::PhantomData<A>);
impl<A> Default for TestEval<A> {
    fn default() -> Self {
        Self(std::marker::PhantomData)
    }
}

impl<'a, A: PathValue> Eval<'a> for TestEval<A> {
    type In = A;
    type Out = Option<ValueString<'a>>;
...
impl<A: PathString> Eval<'_> for ListMustacheEval<A> {
    type In = A;
    type Out = String;

    fn eval(&self, mustache: &Mustache, in_value: &Self::In) -> Self::Out {
        let mut result = String::new(); // Since we're collecting into a String
