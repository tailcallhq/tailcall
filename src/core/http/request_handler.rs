...
async fn execute_query<T: DeserializeOwned + GraphQLRequestLike>(
    ...
    let resp = {
        let _resp = request
            .data(req_ctx.clone())
            .execute(&app_ctx.schema, Some(app_ctx.clone()))
            .await;
        // TODO: impl into_hyper
    ...
    let resp = {
        let response = request
            .data(req_ctx.clone())
            .execute(&app_ctx.schema, None)
            .await;

        let response = update_cache_control_header(response.into(), &app_ctx, req_ctx.clone());
