...
pub fn to_borrowed<'json>(value: &'json ConstValue) -> serde_json_borrow::Value<'json> {
    match value {
        ...
        ConstValue::List(l) => serde_json_borrow::Value::Array(l.into_iter().map(to_borrowed).collect()),
        ConstValue::Object(o) => serde_json_borrow::Value::Object(o.iter().map(|(k, v)| (k.as_str(), to_borrowed(v))).collect::<Vec<_>>().into()),
