...
pub struct GrpcDataLoader {
    ...
    pub(crate) http_client: std::sync::Arc<dyn crate::core::HttpIO>,
}
...
impl GrpcDataLoader {
    ...
    async fn load_dedupe_only(
        ...
        let results = keys.iter().map(|key| async {
            let result = match key.to_request() {
                Ok(req) => execute_grpc_request(&self.http_client, &self.operation, req).await,
                Err(error) => Err(error),
            };
    ...
    async fn load_with_group_by(
        ...
        let response =
            execute_grpc_request(&self.http_client, &self.operation, multiple_request).await?;
