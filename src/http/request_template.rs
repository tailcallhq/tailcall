...
mod tests {
    ...
    fn test_url_query_params() {
        let query = vec![
            ("foo".to_string(), Mustache::parse("0")),
            ("bar".to_string(), Mustache::parse("1")),
            ("baz".to_string(), Mustache::parse("2")),
        ];
    ...
    fn test_url_query_params_template() {
        let query = vec![
            ("foo".to_string(), Mustache::parse("0")),
            ("bar".to_string(), Mustache::parse("{{bar.id}}")),
            ("baz".to_string(), Mustache::parse("{{baz.id}}")),
        ];
        let tmpl = RequestTemplate::new("http://localhost:3000/")
            .unwrap()
            .query(query);
