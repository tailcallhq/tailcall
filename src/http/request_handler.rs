...
async fn handle_rest_apis(
    ...
    *request.uri_mut() = request.uri().path().replace(API_URL_PREFIX, "").parse()?;
    ...
    if let Some((endpoint, p_request)) = app_ctx.endpoints.matches(&request) {
        if let Some(ref rate_limit) = endpoint.rate_limit {
            app_ctx.rate_limiter.allow(&request, rate_limit, ())?;
        }
