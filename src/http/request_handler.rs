...
use super::{request_context::RequestContext, HttpClient};
...
fn create_request_context<Client: HttpClient>(
  req: &Request<Body>,
  server_ctx: &ServerContext<Client>,
...
fn update_cache_control_header<Client: HttpClient>(
  response: GraphQLResponse,
  server_ctx: &ServerContext<Client>,
  req_ctx: Arc<RequestContext<Client>>,
) -> GraphQLResponse {
...
pub fn update_response_headers<Client: HttpClient>(
...
pub async fn graphql_request<T: DeserializeOwned + GraphQLRequestLike, Client: HttpClient>(
  req: Request<Body>,
  server_ctx: &ServerContext<Client>,
...
pub async fn handle_request<T: DeserializeOwned + GraphQLRequestLike, Client: HttpClient>(
  req: Request<Body>,
  state: Arc<ServerContext<Client>>,
) -> Result<Response<Body>> {
  match *req.method() {
    hyper::Method::POST if req.uri().path() == "/graphql" => graphql_request::<T, Client>(req, state.as_ref()).await,
