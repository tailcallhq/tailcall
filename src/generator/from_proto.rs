...
use crate::config::{Arg, Config, Enum, Field, Grpc, Type};
        ...
        for service in services {
            ...
            for method in &service.method {
                let field_name = GraphQLType::new(method.name())
                    .append_namespace(&self.package)
                    .append_namespace(&service_name)
                    .as_method();

                let mut cfg_field = Field::default();
                if let Some(graphql_type) = get_input_type(method.input_type())? {
                    let key = graphql_type.clone().as_field().to_string();
                    let type_of = graphql_type.as_object_type().to_string();
                    let val = Arg {
                        type_of,
                        list: false,
                        required: true,
...
fn graphql_type_from_ref(name: &str) -> Result<GraphQLType<Unparsed>> {
    ...
    if let Some((package, name)) = name.rsplit_once('.') {
        Ok(GraphQLType::new(name).append_namespace(package))
    } else {
