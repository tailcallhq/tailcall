...
object Resiliency {
  ...
  object Threshold {
    ...
    final case class Combine(self: Threshold, other: Threshold, operator: Combine.Operator)
        extends Threshold
    object Combine {
  ...
  object Reset {
    ...
    final case class Exponential(minDuration: Duration, maxDuration: Duration, factor: Int)
        extends Reset
  }
  ...
  object Schedule {
    ...
    final case class Combine(self: Schedule, other: Schedule, operator: Combine.Operator)
        extends Schedule
    object Combine {
      sealed trait Operator
      object Operator {
