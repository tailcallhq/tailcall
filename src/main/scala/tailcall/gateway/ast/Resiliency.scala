...
object Resiliency {
  ...
  final case class CircuitBreaker(threshold: Threshold, reset: Reset)
      extends Resiliency
  ...
  object Threshold {
    ...
    def and(self: Threshold, other: Threshold): Threshold =
      Combine(self, other, Combine.Operator.And)
    def or(self: Threshold, other: Threshold): Threshold  =
      Combine(self, other, Combine.Operator.Or)
  }
  ...
  object RetryPolicy {
    ...
    final case class Exponential(duration: Duration, max: Int, factor: Int)
        extends RetryPolicy
  }

  final case class RateLimiter(duration: Duration, count: Long)
      extends Resiliency
  final case class Retry(policy: RetryPolicy)  extends Resiliency
  final case class BulkHead(maxConcurrency: Long, maxQueue: Long)
      extends Resiliency
  ...
  object Schedule {
    ...
    def and(self: Schedule, other: Schedule): Schedule =
      Combine(self, other, Combine.Operator.And)

    def or(self: Schedule, other: Schedule): Schedule =
      Combine(self, other, Combine.Operator.Or)
  }
}
