...
object EvaluationContext:
  ...
  def live: ZLayer[Any, Nothing, EvaluationContext] = ZLayer.fromZIO(Ref.make(Map.empty[Binding, Any]).map(Live(_)))

  def set(id: Binding, value: Any): ZIO[EvaluationContext, Nothing, Unit] =
    ZIO.serviceWithZIO[EvaluationContext](_.set(id, value))

  def get(id: Binding): ZIO[EvaluationContext, Nothing, Option[Any]] = ZIO.serviceWithZIO[EvaluationContext](_.get(id))

  def drop(id: Binding): ZIO[EvaluationContext, Nothing, Unit] = ZIO.serviceWithZIO[EvaluationContext](_.drop(id))
