...
object Config {
  ...
  final case class ConfigEndpoint(operation: Operation, input: Option[TSchema] = None, output: TSchema)
  ...
  implicit lazy val endpointCodec: JsonCodec[ConfigEndpoint] = { DeriveJsonCodec.gen[ConfigEndpoint] }

  implicit lazy val connectionCodec: JsonCodec[Connection] = { DeriveJsonCodec.gen[Connection] }

  implicit lazy val specificationCodec: JsonCodec[Specification] = { DeriveJsonCodec.gen[Specification] }
