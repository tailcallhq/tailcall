...
trait BooleanOps {
  implicit final class RemoteBooleanOps(val self: Remote[Boolean]) {
    def &&(other: Remote[Boolean]): Remote[Boolean] = Remote(Lambda.and(self.toLambda, other.toLambda))

    def ||(other: Remote[Boolean]): Remote[Boolean] = Remote(Lambda.or(self.toLambda, other.toLambda))
