...
trait MathOps {
  implicit final class Extensions[A, B](val self: A ~> B)(implicit tag: Numeric[B]) {
    def >(other: A ~> B): A ~> Boolean =
      Lambda.unsafe
        .attempt(ctx => Math(Math.Binary(self.compile(ctx), other.compile(ctx), Math.Binary.GreaterThan), tag.any))

    def increment(implicit ctor: Constructor[B]): A ~> B = self + Lambda(tag.one)

    def decrement(implicit ctor: Constructor[B]): A ~> B = self - Lambda(tag.one)
    ...
    def negate: A ~> B = Lambda.unsafe.attempt(ctx => Math(Math.Unary(self.compile(ctx), Math.Unary.Negate), tag.any))
  }
}
