...
object Constructor {
  implicit def primitive[A](implicit s: StandardType[A]): Constructor[A] = Constructor(Schema.primitive(s))
  ...
  implicit val dynamicValue: Constructor[DynamicValue] = Constructor(Schema[DynamicValue])

  implicit val context: Constructor[Context] = Constructor(Schema[Context])

  implicit def tuple2[A1, A2](implicit ctor1: Constructor[A1], ctor2: Constructor[A2]): Constructor[(A1, A2)] = {
    implicit val schema1: Schema[A1] = ctor1.schema
    implicit val schema2: Schema[A2] = ctor2.schema
    Constructor(Schema[(A1, A2)])
  }
  implicit def map[A, B](implicit ctorA: Constructor[A], ctorB: Constructor[B]): Constructor[Map[A, B]]       = {
