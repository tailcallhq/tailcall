...
import tailcall.gateway.lambda.{Lambda, LambdaRuntime, Remote}
...
final class StepGenerator(orc: Orc) {
  ...
  def gen(orc: Orc, context: Remote[Context]): RemoteStep = {
    orc match {
      ...
      case Orc.OrcFunction(fun) => Step.QueryStep(ZQuery.fromZIO(fun(context).evaluateWith(()).map(gen(_, context))))
    }
  }
