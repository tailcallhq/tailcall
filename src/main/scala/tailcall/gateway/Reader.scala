...
object Reader {
  ...
  def config: Reader[Config] = new Reader[Config] {
  ...
  def document: Reader[Document] = new Reader[Document] {
    override def readFile(file: => File): Task[Document] = {
      for {
        ...
        rootSchemaBuilder <- caliban.tools.RemoteSchema.parseRemoteSchema(document) match {
          case None           => ZIO
              .fail(new RuntimeException("GraphQL does not contain a schema definition"))
