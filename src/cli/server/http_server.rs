...
impl Server {
    ...
    pub async fn start(self) -> Result<()> {
        ...
        let runtime_config = RuntimeConfig::try_from(&self.config_module).map_err(Errata::from)?;
        let server_config =
            Arc::new(ServerConfig::new(blueprint.clone(), runtime_config, endpoints).await?);

        init_opentelemetry(
            server_config.app_ctx.runtime_config.telemetry.clone(),
            &server_config.app_ctx.runtime,
        )?;

        match server_config.app_ctx.runtime_config.server.http.clone() {
            Http::HTTP2 { cert, key } => {
