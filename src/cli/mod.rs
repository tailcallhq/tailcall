...
pub fn init_hook_http(http: impl HttpIO, script: Option<ScriptOptions>) -> Arc<dyn HttpIO> {
...
pub fn init_http(upstream: &Upstream, script: Option<ScriptOptions>) -> Arc<dyn HttpIO> {
  let http_io = http::NativeHttp::init(upstream);
  init_hook_http(http_io, script)
}

// Provides access to http in native rust environment
pub fn init_http2_only(upstream: &Upstream, script: Option<ScriptOptions>) -> Arc<dyn HttpIO> {
...
pub fn init_script(script: ScriptOptions) -> Arc<dyn ScriptIO<Event, Command>> {
