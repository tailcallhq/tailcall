...
impl Generator {
    ...
    async fn write(self, graphql_config: &ConfigModule, output_path: &str) -> anyhow::Result<()> {
        let output_source = config::SourceType::detect(output_path)?;
        let config = match output_source {
            config::SourceType::Json => graphql_config.to_json(true)?,
            config::SourceType::Yml => graphql_config.to_yaml()?,
            config::SourceType::GraphQL => graphql_config.to_sdl(),
        };
