    ...
    Box::pin(async move {
      Ok(match self {
        ...
        Relation::PathEq(lhs, path, rhs) => {
          let lhs = lhs.eval(ctx, conc).await?;
          let lhs =
            get_path_for_const_value_owned(path, lhs).ok_or(anyhow::anyhow!("Could not find path: {path:?}"))?;

          let rhs = rhs.eval(ctx, conc).await?;
          let rhs =
            get_path_for_const_value_owned(path, rhs).ok_or(anyhow::anyhow!("Could not find path: {path:?}"))?;
