...
use super::{
  eval_list_expressions, Concurrency, Eval, EvaluationContext, EvaluationError, Expression, ResolverContextLike,
};
...
pub async fn eval_math<'a, Ctx: ResolverContextLike<'a> + Sync + Send>(
  ...
  Ok(match math {
    ...
    Math::Product(exprs) => {
      let results: Vec<_> = super::eval_list_expressions(ctx, conc, exprs).await?;
