...
pub struct ValidationError<E>(BTreeSet<Cause<E>>);

impl<E: Display + Ord> Display for ValidationError<E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("Validation Error\n")?;
        let errors = self.as_set();
        for error in errors {
...
impl<E: std::cmp::Ord> ValidationError<E> {
    pub fn as_set(&self) -> &BTreeSet<Cause<E>> {
        &self.0
    }
    ...
    pub fn trace(self, message: &str) -> Self {
        ...
        let errors = BTreeSet::from_iter(errors);
        Self(errors)
    }
...
impl<E: std::cmp::Ord> From<Cause<E>> for ValidationError<E> {
...
impl<E: std::cmp::Ord> From<Vec<Cause<E>>> for ValidationError<E> {
