...
object Main extends ZIOAppDefault {
  ...
  override val run = GraphQLConfig.bootstrap { config =>
    Server.install(server).flatMap(port => ZIO.log(s"Server started: http://localhost:${port}/graphql") *> ZIO.never)
      .provide(
        ZLayer.succeed(Server.Config.default.port(config.port).disableRequestStreaming(Int.MaxValue)),
  ...
  private val server = (AdminServer.rest ++ Http.collectHttp[Request] {
    case Method.POST -> Root / "graphql"     => AdminServer.graphQL
    case Method.POST -> Root / "graphql" / _ => GenericServer.graphQL
  ...
  private def jsonError(message: String, status: Status = Status.InternalServerError): Response = {
    ...
    Response.json(response.toJson).withStatus(status)
  }
}
