...
mod integration_tests {
  ...
  async fn verify_response_headers() {
    ...
    let schema_path = "tests/e2e/configs/test-custom-headers.graphql";
  ...
  #[tokio::test]
  async fn verify_cache_control_headers() {
    let schema_path = "tests/e2e/configs/test-cache-control-headers.graphql";
    // Start the background server with the provided schema.
    tokio::spawn(initiate_test_server(schema_path.into()));
    ...
    let response_headers = response.headers();
    assert_eq!(response_headers.get("cache-control").unwrap(), "max-age=43200");
  }
}
