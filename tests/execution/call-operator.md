...
```graphql @server
schema
  @server(graphiql: true, hostname: "0.0.0.0", port: 8000)
...
type Query {
  ...
  newsWithPortArg(port: Int!): NewsData!
    @grpc(baseURL: "http://localhost:{{args.port}}", method: "news.NewsService.GetAllNews")
  ...
  userGraphQLHeaders(id: Int!): User
    @graphQL(baseURL: "http://upstream/graphql", headers: [{key: "id", value: "{{args.id}}"}], name: "user")
