...
type Query {
  ...
  newsById(news: NewsInput!): News! @grpc(url: "http://localhost:50051", method: "news.NewsService.GetNews", body: "{{.args.news}}")
}

type Mutation {
  deleteNews(news: NewsId!): Empty! @grpc(url: "http://localhost:50051", method: "news.NewsService.DeleteNews", body: "{{.args.news}}")
}
