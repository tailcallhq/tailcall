...
impl ConfigLoader {
  ...
  pub async fn start_polling(&mut self, batch_single: Arc<ServerContext>, batch_state: Arc<ServerContext>) {
    loop {
      ...
      if let Ok(bp) = Blueprint::try_from(&conf) {
        let old_sdl = batch_state.schema.read().await.sdl();
        ...
        if old_sdl.ne(&new_schema.sdl()) {
          ...
          *batch_state.schema.write().await = new_schema;
        } else {
