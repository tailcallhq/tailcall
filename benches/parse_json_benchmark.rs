...
pub fn parse_json(file_path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let benchmarks: Vec<Benchmark> = content
        .lines()
...
fn print_table(benchmarks: &[Benchmark]) {
    ...
    for benchmark in benchmarks {
        ...
        let formatted_estimate =
            format_value(rounded_typical.estimate, &rounded_typical.unit);
        let formatted_lower_bound =
            format_value(rounded_typical.lower_bound, &rounded_typical.unit);
        let formatted_upper_bound =
            format_value(rounded_typical.upper_bound, &rounded_typical.unit);
    ...
    fs::write(_output_file_path, markdown_table)
        .expect("Failed to write Markdown table to file");
    println!(
        "Markdown table (Typical values) written to {}",
        _output_file_path
    );
}
