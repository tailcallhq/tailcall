  ...
  def spec =
    suite("HttpCacheSpec Cache-Control")(
      ...
      test("expires after 1000 second") {
        //
        val p        = for {
          now        <- ZIO.succeed(Instant.parse("2021-01-01T00:00:00Z"))
          headerTime <- ZIO
            .succeed(new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss z").format(now.toEpochMilli + 1000000L))
          _          <- TestClock.setTime(now)
        } yield HttpCache.ttl(Response.ok.addHeaders(headers = Header("expires", headerTime)), now)
        val expected = Some(Duration.fromSeconds(1000))
        assertZIO(p)(equalTo(expected))
      },
    )
