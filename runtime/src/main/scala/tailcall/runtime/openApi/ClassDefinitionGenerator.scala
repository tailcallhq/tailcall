...
import tailcall.runtime.openApi.BasicGenerator.{indent, mapSchemaSimpleTypeToType}
import tailcall.runtime.openApi.OpenapiModels.OpenapiDocument
import tailcall.runtime.openApi.OpenapiSchemaType.{OpenapiSchemaArray, OpenapiSchemaObject, OpenapiSchemaSimpleType}

class ClassDefinitionGenerator {

  def classDefs(doc: OpenapiDocument): String = {
    val classes = doc.components.schemas.flatMap {
      case (name, obj: OpenapiSchemaObject) => generateClass(name, obj)
      case _                                => throw new NotImplementedError("Only objects supported!")
    }
    classes.mkString("\n")
  }

  private[openApi] def generateClass(name: String, obj: OpenapiSchemaObject): Seq[String] = {
    def addName(parentName: String, key: String)                                    =
    ...
    def rec(name: String, obj: OpenapiSchemaObject, acc: List[String]): Seq[String] = {
      val innerClasses = obj.properties.collect { case (propName, st: OpenapiSchemaObject) =>
  ...
  private val reservedKeys = scala.reflect.runtime.universe.asInstanceOf[scala.reflect.internal.SymbolTable].nme
    .keywords.map(_.toString)

  private def fixKey(key: String) = { if (reservedKeys.contains(key)) s"`$key`" else key }
