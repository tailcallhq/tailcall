...
import tailcall.runtime.openApi.OpenapiModels.{
  OpenapiDocument,
  OpenapiParameter,
  OpenapiPath,
  OpenapiRequestBody,
...
class EndpointGenerator {
  ...
  private[openApi] def allEndpoints: String = "generatedEndpoints"

  def endpointDefs(doc: OpenapiDocument): String = {
    val ge          = doc.paths.flatMap(generatedEndpoints)
    val definitions = ge.map { case (name, definition) =>
      s"""|val $name =
          |${indent(2)(definition)}
          |""".stripMargin
    }.mkString("\n")
  ...
  private[openApi] def generatedEndpoints(p: OpenapiPath): Seq[(String, String)] = {
    p.methods.map { m =>
      val definition = s"""|endpoint
                           |  .${m.methodType}
      ...
      val name = m.methodType + p.url.split('/').map(_.replace("{", "").replace("}", "").toLowerCase.capitalize)
        .mkString
      (name, definition)
    }
  }
  ...
  private def ins(parameters: Seq[OpenapiParameter], requestBody: Option[OpenapiRequestBody]): String = {
    ...
    val params = parameters.filter(_.in != "path").map { param =>
      param.schema match {
        case st: OpenapiSchemaSimpleType =>
          val (t, _) = mapSchemaSimpleTypeToType(st)
  ...
  private def outs(responses: Seq[OpenapiResponse]) = {
    ...
    responses.map { resp =>
      ...
      resp.code match {
        ...
        case _         => throw new NotImplementedError("Statuscode mapping is incomplete!")
      }
    }.sorted.mkString("\n")
  }
